function [U,C] = ur_gmres_harmritz_recycling(A,param,L,V,H)

%% Function Definition %%
% UR_GMRES_HARMRITZ_RECYCLING - A function which construct a recycling
% subspace for urGMRES using an augmented Krylov subspace K + U, by solving
...
%    - C, matrix such that C = A*U;
    ...
    A1 = zeros(k+m);
    A1(1:k,1:k) = CTU;
    A1(1:k,k+1:end) = C'*V(:,1:m);
    A1(k+1:end,1:k) = (V*H)'*U;
    A1(k+1:end,k+1:end) = H(1:m,1:m)';

    A2 = eye(k+m);
    A2(1:k,k+1:end) = L;
    A2(k+1:end,1:k) = L';
    A2(k+1:end,k+1:end) = H'*H;
    ...
    % Impose orthonormal columns of C for urGMRES
    [C,RR] = qr(C,0);
    U = U/RR;
